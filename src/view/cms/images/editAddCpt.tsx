      ...
      ajax
        ...
        .then((res) => {
          ...
          form.setFieldsValue({
            ...
            urls: data.urls
              ? data.urls.split(",").map((itme: any) => {
                  return { url: itme, uid: signRonder(6) };
                })
              : [],
          });
          setLoading(false);
...
function useGetClassify() {
  ...
  const getClassifyList = () => {
    ajax
      .GET("cmsClassify", { isPage: true })
      .then((res) => {
        let { data } = res.data;
        setClassifyList(
          oneToTree(data.rows, "id", "pid", 0, 1, (itme: any) => {
            itme["disabled"] = itme.status === 1 ? false : true;
          })
        );
      })
      .catch((err) => {});
...
const EditAddCpt = () => {
  ...
  const onFinish = () => {
    ...
    form.validateFields().then((value) => {
      ...
      let postData = {
        ...
        urls: value.urls
          ? value.urls.map((itme: any) => itme.url).join(",")
          : "",
      };
  ...
  return (
    <div className="page-images" style={{ maxWidth: 960, margin: "auto" }}>
      <Spin spinning={loading}>
        ...
        >
          ...
          <Form.Item label="图片集">
            ...
            <Form.Item
              label={<Button onClick={insertNetImgFn}>插入网络图片</Button>}
              name="netimg"
            ...
            <Tooltip title="多张用英文逗号分隔开">
              <span style={{ color: "#999" }}>
                <ExclamationCircleTwoTone style={{ color: "#999" }} />{" "}
                &nbsp;插入提示
              </span>
            </Tooltip>
          </Form.Item>
          ...
          <div className=" by-fixed-footer">
            <div
              className="m-flex m-center operation-main"
              style={{ maxWidth: 1150, paddingLeft: "8%" }}
            >
              ...
              <Button
                type="primary"
                onClick={() => History.push("/admin/cmsimage")}
              >
                {" "}
                返回{" "}
              </Button>
