...
// import {routerData,IRouterData} from "@/router/datas"

import { treeToOneArr } from "@/util";
import { authRoutes } from "@/router/data/authRoutes";
import { IRouter } from "@/router/interface";
const { TabPane } = Tabs;
const allRouter = treeToOneArr(authRoutes);
...
class LayoutTabs extends Component<Iprops> {
  ...
  filterRouter(path: string) {
    let pathItme = allRouter.filter(
      (itme) => setPathKey(itme.path || "") === setPathKey(path) && itme.title
    );
    return pathItme[0] ? pathItme[0] : null;
  }
  addTabs = (item: IRouter) => {
  ...
  isRouteTabs(key: string) {
    // ctx.path.replace(/(\/)?$/,"/");
    var pathItme = this.state.panes.filter(
      (itme) => setPathKey(itme.key) === setPathKey(key)
    );
